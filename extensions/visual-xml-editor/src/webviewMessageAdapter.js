 ...
 */
export async function processWebviewMessage(e, document, webviewPanel, serializer, output, _context) {
    try {
        switch (e.type) {
            case "edit": {
                const incoming = e.content;
                try {
                    output.appendLine("adapter: webview -> edit (received)");
                }
                catch { }
                ...
                try {
                    output.appendLine("adapter: applyEdit invoked");
                }
                catch { }
                break;
            }
            case "incremental": {
                try {
                    output.appendLine("adapter: incremental message received (not implemented)");
                }
                catch { }
                // Inform the webview that incremental is not yet applied
                try {
                    webviewPanel.webview.postMessage({ type: "saveAck", status: "error", details: "incremental not implemented yet" });
                }
                catch { }
                break;
            }
            case "fullDocument": {
                ...
                await vscode.workspace.applyEdit(edit);
                try {
                    webviewPanel.webview.postMessage({ type: "saveAck", status: "ok" });
                }
                catch { }
                break;
            }
            case "requestSave": {
                ...
                try {
                    webviewPanel.webview.postMessage({ type: "requestFullDocument" });
                }
                catch { }
                break;
            }
            default: {
                try {
                    output.appendLine("adapter: unknown message type: " + String(e.type));
                }
                catch { }
            }
        }

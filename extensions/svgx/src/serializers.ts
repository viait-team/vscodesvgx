...
export function createSVGXCellFromNotebookCell(
 ...
 * SVGX notbeooks/labs sorts the JSON keys in alphabetical order.
...
function translateCellDisplayOutput(output: NotebookCellOutput): SVGXOutput {
	const customMetadata = output.metadata as CellOutputMetadata | undefined;
	let result: SVGXOutput;
	// Possible some other extension added some output (do best effort to translate & save in svgx).
	...
	switch (outputType) {
		...
		case 'display_data': {
			result = {
				...
				data: output.items.reduce((prev: any, curr) => {
					prev[curr.mime] = convertOutputMimeToSVGXOutput(curr.mime, curr.data as Uint8Array);
					return prev;
				}, {}),
		...
		default: {
			...
			// In the case of .NET & other kernels, we need to ensure we save svgx correctly.
			// Hence if we have stream output, save the output as SVGX `stream` else `display_data`
...
function translateCellErrorOutput(output: NotebookCellOutput): nbformat.IError {
	...
	return {
		...
		// stack is empty, hence store the message instead of stack (so that somethign gets displayed in svgx).
...
function convertStreamOutput(output: NotebookCellOutput): SVGXOutput {
...
function convertOutputMimeToSVGXOutput(mime: string, value: Uint8Array) {
	if (!value) {
		return '';
	}
	try {
		if (mime === CellOutputMimeTypes.error) {
			...
			// Images in SVGX are stored in base64 encoded format.
...
export function serializeNotebookToString(data: NotebookData): string {
	...
	notebookContent.cells = data.cells
		.map(cell => createSVGXCellFromNotebookCell(cell, preferredCellLanguage))
		.map(pruneCell);
...
function serializeNotebookToJSON(notebookContent: Partial<nbformat.INotebookContent>, indentAmount: string): string {
	// svgx always ends with a trailing new line (we add this so that SCMs do not show unnecessary changes, resulting from a missing trailing new line).

...
/**
 * Code here is used to ensure the Notebook Model is in sync the svgx JSON file.
 * E.g. assume you add a new cell, this new cell will not have any metadata at all.
 * However when we save the svgx, the metadata will be an empty object `{}`.
...
*
* This code ensures that the model is in sync with the svgx file.
*/
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	// Else model will be out of sync with svgx https://github.com/microsoft/vscode/issues/207968#issuecomment-2002858596
	...
	e.contentChanges.forEach(change => {
		change.addedCells.forEach(cell => {
			...
			// as metadata is always an empty `{}` in svgx JSON file

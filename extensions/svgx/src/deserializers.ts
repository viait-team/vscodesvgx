...
const svgxLanguageToMonacoLanguageMapping = new Map([
	['c#', 'csharp'],
	['f#', 'fsharp'],
	['q#', 'qsharp'],
	['c++11', 'c++'],
...
export function getPreferredLanguage(metadata?: nbformat.INotebookMetadata) {
	...
	return translateKernelLanguageToMonaco(svgxLanguage || defaultLanguage);
}
...
function translateKernelLanguageToMonaco(language: string): string {
	...
	return svgxLanguageToMonacoLanguageMapping.get(language) || language;
}
...
function convertSVGXOutputToBuffer(mime: string, value: unknown): NotebookCellOutputItem {
	if (!value) {
	...
	*/
	...
	if (output.data) {
		for (const key in output.data) {
			items.push(convertSVGXOutputToBuffer(key, output.data[key]));
		}
	}
...
export function svgxCellOutputToCellOutput(output: nbformat.IOutput): NotebookCellOutput {
	/**
...
function createNotebookCellDataFromCodeCell(cell: nbformat.ICodeCell, cellLanguage: string): NotebookCellData {
	...
	const outputs = cellOutputs.map(svgxCellOutputToCellOutput);
...
function createNotebookCellDataFromSVGXCell(
	cellLanguage: string,
 ...
 */
export function svgxNotebookModelToNotebookData(
	...
	const cells = notebookContent.cells
		.map(cell => createNotebookCellDataFromSVGXCell(preferredLanguage, cell))

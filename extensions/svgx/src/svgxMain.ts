...
export function activate(context: vscode.ExtensionContext, serializer: vscode.NotebookSerializer) {
	...
	context.subscriptions.push(vscode.workspace.registerNotebookSerializer('svgx-notebook', serializer, notebookSerializerOptions));
	...
	vscode.languages.registerCodeLensProvider({ pattern: '**/*.svgx' }, {
		provideCodeLenses: (document) => {
			...
			const codelens = new vscode.CodeLens(new vscode.Range(0, 0, 0, 0), { title: 'Open in Notebook Editor', command: 'svgx.openSvgxInNotebookEditor', arguments: [document.uri] });
			return [codelens];
		}
	});

	context.subscriptions.push(vscode.commands.registerCommand('svgx.newUntitledSvgx', async () => {
		...
		const doc = await vscode.workspace.openNotebookDocument('svgx-notebook', data);
		await vscode.window.showNotebookDocument(doc);
	}));

	context.subscriptions.push(vscode.commands.registerCommand('svgx.openSvgxInNotebookEditor', async (uri: vscode.Uri) => {
	...
	const enabled = vscode.workspace.getConfiguration('svgx').get('pasteImagesAsAttachments.enabled', false);
	if (enabled) {
